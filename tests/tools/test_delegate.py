...
from agent import AgentSpec, RoutingEnvelope, pick_lane, pick_route, validate_output_contract, validate_routing_envelope, WorkflowOrchestrator
...
class TestChildSystemPrompt(unittest.TestCase):
    ...
    def test_routing_metadata_injected(self):
        ...
        self.assertIn("[AGENT_SPEC]", prompt)
        self.assertIn("agent_id=worker-1", prompt)
...
class TestRoutingAndWorkflow(unittest.TestCase):
    def test_pick_lane_and_route(self):
        spec = {
            "agent_id": "exec-1",
            "role": "worker",
            "persona": "code",
    ...
    def test_validate_routing_envelope(self):
        env = {
            "agent_spec": {
                "agent_id": "rev-1",
                "role": "verifier",
        ...
        validate_routing_envelope(env)

    def test_validate_output_contract(self):
        validate_output_contract("summary-v1", {"summary": "ok", "next_action": "ship"})
        with self.assertRaises(ValueError):
            validate_output_contract("review-report-v1", {"verdict": "maybe", "evidence": []})
    ...
    def test_workflow_orchestrator(self):
        ...
        orch = WorkflowOrchestrator(
            ...
            persist_fn=lambda node_id, result: persisted.append((node_id, result)),
        ...
        out = orch.run(workflow, {"task": "hi"})
        self.assertEqual(out["summary"], "n1")
        self.assertEqual(len(calls), 1)
        self.assertEqual(persisted[0][0], "n1")
