...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        # Re-read model.context_length from fresh config so the top-level
        # override survives model switch even when _config_context_length
        # was cleared.  Previously, clearing to None + no per-model
        # custom_provider override caused combo models without metadata
        # (e.g. 9router auto) to fall through to the 256K default.
        _switch_config_ctx = getattr(agent, "_config_context_length", None)
        if _switch_config_ctx is None and isinstance(_sm_cfg, dict):
            try:
                _m_cfg = _sm_cfg.get("model", {})
                if isinstance(_m_cfg, dict):
                    _raw = _m_cfg.get("context_length")
                    if _raw is not None:
                        _switch_config_ctx = int(_raw)
        ...
        # Persist the re-read config context_length so subsequent switch
        # calls don't need to re-parse config.yaml each time.
        if _switch_config_ctx is not None:
            agent._config_context_length = _switch_config_ctx
